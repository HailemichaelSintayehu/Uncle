  ...
  reducers: {
    cart_product: (state, { payload }: PayloadAction<ProductsType>) => {
      const productIndex = state.cartProducts.findIndex(
        (item) => item.id === payload.id && item.option === payload.option
      );
    ...
    remove_cart_product: (state, { payload }: PayloadAction<ProductsType>) => {
      ...
      const updatedCartProducts = state.cartProducts.filter(
        (item) => !(item.option === payload.option && item.id === payload.id)
      );
      state.cartProducts = updatedCartProducts;
      toast.success(`Removed from your cart`, { id: toastId, duration: 2000 });
    },
    update_cart_product: (state, { payload }: PayloadAction<ProductsType>) => {
      const updatedCartProducts = state.cartProducts?.map((item) => ({ ...item, ...payload }));
      state.cartProducts = updatedCartProducts;
    },
    update_cart_product_refundable: (state, { payload }: PayloadAction<ProductsType>) => {
      const updatedCartProducts = payload as any;
      state.cartProducts = updatedCartProducts;
    },
    ...
    decrease_quantity: (state, { payload }: PayloadAction<ProductsType>) => {
      ...
      const cartIndex = state.cartProducts.findIndex(
        (item) => item.option === payload.option
      );
...
export const {
  ...
  decrease_quantity,
  update_cart_product,
  update_cart_product_refundable,
} = cartSlice.actions;
